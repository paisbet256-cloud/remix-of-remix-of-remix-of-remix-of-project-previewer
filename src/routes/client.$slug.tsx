...
function ClientReportPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Show AD SET level data here (not ad-level) — matches Ads Manager's
      // ad-set table that the user actually reads day-to-day.
      let adSets: any[] = [];
      if (campIds.length) {
        const { data } = await supabase
          .from("ad_sets")
          .select("id,name,fb_adset_id,effective_status,campaign_id,ad_account_id,daily_budget,lifetime_budget,spend,impressions,reach,clicks,results")
      ...
      return { client, accounts: accounts ?? [], campaigns, adSets, acctById };
    },
  });
  ...
  const { client, accounts, campaigns, adSets, acctById } = data;
      ...
      <div className="grid gap-4 lg:grid-cols-[260px_1fr]">
        ...
        <div className="glass-card overflow-hidden">
          <div className="px-4 py-3 border-b border-border/40 text-xs text-emerald-400">
            ✓ Loaded {adSets.length} assigned ad set{adSets.length !== 1 ? "s" : ""}
          </div>
          <div className="px-4 py-3 border-b border-border/40 flex items-center justify-between">
            <div className="font-semibold">Assigned Ad Sets</div>
            <div className="text-xs text-muted-foreground">{adSets.length} AD SET{adSets.length !== 1 ? "S" : ""}</div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[11px] uppercase tracking-wider text-muted-foreground bg-surface/40">
                  <th className="text-left px-4 py-2.5">Ad Set Name</th>
                  <th className="text-left px-4 py-2.5">Ad Set ID</th>
                  <th className="text-left px-4 py-2.5">Ad Account</th>
                  <th className="text-left px-4 py-2.5">Status</th>
                  <th className="text-right px-4 py-2.5">Daily Budget</th>
              ...
              <tbody>
                {adSets.length === 0 ? (
                  <tr><td colSpan={7} className="text-center py-12 text-muted-foreground text-sm">No assigned ad sets.</td></tr>
                ) : adSets.map((a: any) => {
                  const acct: any = acctById.get(a.ad_account_id);
                  const daily = Number(a.daily_budget) || 0;
                  return (
                    <tr key={a.id} className="border-t border-border/40 hover:bg-surface/40">
                      ...
                      <td className="px-4 py-3"><code className="text-xs font-mono text-muted-foreground">{a.fb_adset_id}</code></td>
                      ...
                      <td className="px-4 py-3 text-right tabular-nums text-muted-foreground">{daily > 0 ? fmtUSD(daily) : "—"}</td>
