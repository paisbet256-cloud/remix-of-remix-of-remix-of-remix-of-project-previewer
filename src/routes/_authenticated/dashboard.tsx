...
import {
  DollarSign, Users, CheckSquare, Megaphone,
  RefreshCw, Loader2, ShieldCheck, Trash2,
} from "lucide-react";
...
const PIE_COLORS = [
  "oklch(0.78 0.18 165)", "oklch(0.66 0.22 295)", "oklch(0.83 0.16 85)",
  "oklch(0.70 0.20 30)", "oklch(0.62 0.18 220)",
];
...
// ✅ Fix #4 helper: account-currency spend → USD (Fix #3 এর সাথে consistent)
function accountSpendToUsd(spend: number, currency: string | null | undefined, bdtRate: number | null | undefined): number {
  if (!spend || !isFinite(spend)) return 0;
  const cur = (currency ?? "USD").toUpperCase();
  if (cur === "USD") return spend;
  if (cur === "BDT") {
    const rate = Number(bdtRate);
    if (!rate || rate <= 0) return 0;
    return spend / rate; // bdt_rate = BDT per 1 USD
  }
  return spend; // অন্য currency কে USD ধরা হলো (future: rates table)
}
...
function Dashboard() {
  ...
  // ✅ Fix #4: client এর bdt_rate + deposit_currency সহ select
  const { data: accounts } = useQuery({
    queryKey: ["dashboard-accounts"],
    queryFn: async () => {
      const { data } = await supabase
        .from("ad_accounts")
        .select("*, client:clients(id,name,slug,bdt_rate,deposit_currency)")
        .eq("is_active", true)
  ...
  const { data: timeSeries } = useQuery({
    ...
    queryFn: async () => {
      const days =
        datePreset === "Today" || datePreset === "Yesterday" ? 1
        : datePreset === "Last 14 days" ? 14
        : datePreset === "Last 30 days" ? 30 : 7;

      const cutoff = new Date();
      if (datePreset === "Yesterday") cutoff.setDate(cutoff.getDate() - 1);
      else if (datePreset !== "Today") cutoff.setDate(cutoff.getDate() - (days - 1));
  ...
  useEffect(() => {
    if (!autoSync || !settings?.has_token) { setAutoSyncErrors(0); return; }
    ...
    return () => { isMounted = false; window.clearInterval(timer); window.clearTimeout(initialTimer); };
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .subscribe();
    return () => { isActive = false; supabase.removeChannel(ch); };
  }, [qc]);

  // ✅ Fix #4: timeSeries খালি/0 হলে ad_accounts থেকে fallback (currency-correct)
  const totals = useMemo(() => {
    const acc = Array.isArray(accounts) ? accounts : [];
    const ts = Array.isArray(timeSeries) ? timeSeries : [];

    const rangeTotals = ts.reduce(
      (s: any, r: any) => ({
    ...
    // Fallback: ad_accounts.total_* (account-currency → USD)
    const accFallback = acc.reduce(
      (s: any, a: any) => {
        const rate = a?.client?.bdt_rate ?? null;
        const spendUsd = accountSpendToUsd(Number(a?.total_spend) || 0, a?.currency, rate);
        return {
          spend: s.spend + spendUsd,
          reach: s.reach + (Number(a?.total_reach) || 0),
          results: s.results + (Number(a?.total_results) || 0),
        };
      },
      { spend: 0, reach: 0, results: 0 },
    );
    ...
    return {
      spend: rangeTotals.spend > 0 ? rangeTotals.spend : accFallback.spend,
      reach: rangeTotals.reach > 0 ? rangeTotals.reach : accFallback.reach,
      results: rangeTotals.results > 0 ? rangeTotals.results : accFallback.results,
  ...
  const onSync = async () => {
    ...
    try {
      const res = await syncFn({ data: undefined });
      if (res.skipped) toast.error(res.tokenHealth?.error ?? "Token check failed.");
      else {
        qc.invalidateQueries({ queryKey: ["dashboard-accounts"] });
        qc.invalidateQueries({ queryKey: ["dashboard-timeseries"] });
        qc.invalidateQueries({ queryKey: ["dashboard-logs"] });
  ...
  const onRefreshAllConfirmed = async () => {
    setShowRefreshConfirm(false); setRefreshingAll(true);
    try {
    ...
    <div className="space-y-6 gv-fade-up">
      {showRefreshConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="glass-card p-6 max-w-sm w-full mx-4 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">এটা সব cached data মুছে ফেলবে এবং পুরো re-sync করবে।</p>
            <div className="flex gap-3">
              <button onClick={onRefreshAllConfirmed} className="flex-1 rounded-xl bg-destructive text-white px-4 py-2 text-sm font-semibold hover:bg-destructive/90">হ্যাঁ, করুন</button>
              <button onClick={() => setShowRefreshConfirm(false)} className="flex-1 rounded-xl bg-surface-elevated border border-border px-4 py-2 text-sm font-medium hover:bg-surface">বাতিল</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="relative z-10 flex flex-col lg:flex-row gap-6">
          <div className="flex-1">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              <button onClick={onRetest} disabled={retesting} className="inline-flex items-center gap-2 rounded-xl bg-surface-elevated border border-border px-4 py-2.5 text-sm font-medium hover:bg-surface disabled:opacity-50">
                {retesting ? <Loader2 className="size-4 animate-spin" /> : <ShieldCheck className="size-4" />} Re-test & Re-import
              </button>
              <button onClick={() => setShowRefreshConfirm(true)} disabled={refreshingAll} className="inline-flex items-center gap-2 rounded-xl bg-destructive/10 border border-destructive/40 text-destructive px-4 py-2.5 text-sm font-medium hover:bg-destructive/20 disabled:opacity-50">
      ...
      <div className="grid gap-4 lg:grid-cols-3 gv-stagger">
        <div className="glass-card p-5 lg:col-span-2">
          <CandlestickChart
            data={(timeSeries ?? []) as any} height={260}
            series={[{ key: "spend", label: "Spend", upColor: "oklch(0.78 0.18 165)", downColor: "oklch(0.66 0.22 25)", format: (v) => `$${v.toFixed(2)}` }]}
          />
        </div>
