...
-- Trigger now that user_roles exists
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- ============ APP SETTINGS (server-only token storage) ============
...
-- No GRANT to authenticated — only service_role accesses (via server fn)
GRANT ALL ON public.app_settings TO service_role;
ALTER TABLE public.app_settings ENABLE ROW LEVEL SECURITY;
INSERT INTO public.app_settings (id) VALUES (1) ON CONFLICT DO NOTHING;

-- A safe public view: returns whether token exists + sync config (no actual token)
...
CREATE TABLE public.clients (
  ...
  logo_url TEXT,
  brand_color TEXT,
  portal_password TEXT,   -- optional password (not used by default)
...
CREATE TABLE public.ad_accounts (
  ...
  fb_account_id TEXT NOT NULL UNIQUE, -- act_xxxxxxxxx
  account_name TEXT,
  currency TEXT,
  timezone_name TEXT,
  account_status INT,
  business_name TEXT,
  -- cached aggregates (last sync)
...
-- ============ INSIGHTS SNAPSHOTS (time-series) ============
