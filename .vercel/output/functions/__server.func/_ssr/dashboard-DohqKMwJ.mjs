...
import { d as retestAndReimport, g as syncAllAccountsNow, o as getSettingsPublic, u as refreshAllData } from "./admin.functions-D-0P87qP.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/dashboard-DohqKMwJ.js
...
function Dashboard() {
	...
	const { data: settings } = useQuery({
		queryKey: ["settings"],
		queryFn: () => getSettings({ data: void 0 })
	});
	...
	(0, import_react.useEffect)(() => {
		...
		const timer = window.setInterval(async () => {
			if (syncing) return;
			try {
				if (!(await syncFn({ data: void 0 })).skipped) qc.invalidateQueries();
			} catch (e) {
	...
	(0, import_react.useEffect)(() => {
		const ch = supabase.channel("dashboard-rt").on("postgres_changes", {
			...
			schema: "public",
			table: "ad_accounts"
		}, () => qc.invalidateQueries({ queryKey: ["dashboard-accounts"] })).on("postgres_changes", {
			event: "*",
			schema: "public",
			table: "sync_logs"
		}, () => qc.invalidateQueries({ queryKey: ["dashboard-logs"] })).subscribe();
		return () => {
	...
	const onSync = async () => {
		...
		setSyncing(true);
		try {
			const res = await syncFn({ data: void 0 });
